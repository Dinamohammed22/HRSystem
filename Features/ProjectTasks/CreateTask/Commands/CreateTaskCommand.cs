...
namespace EasyTask.Features.ProjectTasks.CreateTask.Commands
{
    public record CreateTaskCommand
        (string Name, DateTime StartDate, DateTime EndDate, string ProjectId, string WorkPackageId, string? ProjectListId,
    ...
    public class CreateTaskCommandHandler : RequestHandlerBase<ProjectTask, CreateTaskCommand, string>
    {
        ...
        {
            var projectListId = request.ProjectListId ?? (await _mediator.Send(new GetDefaultProjectListIdQuery(request.ProjectId))).Data;

            if (string.IsNullOrEmpty(projectListId))
